...
https://task-board-assignment-chi.vercel.app/

## GitHub 저장소

https://github.com/cokid1998/task-board-assignment
...
- **로드 상태 처리**: 로딩 / 에러(재시도 버튼) / 빈 상태를 명확히 구분
- **낙관적 업데이트와 실패 롤백**: 이동·생성·수정·삭제 전부 UI를 먼저 반영한 뒤 서버에 요청, 실패 시 정확히 롤백 + 알림
- **경쟁 상태(race condition) 처리**: 카드별 mutationId(순번)와 시퀀스 시작 시점의 original(원본)을 관리하여 낡은 응답이 최신 화면을 덮어쓰지 않도록 방어
- **대량 데이터 성능(5,000개)**: React.memo로 카드 컴포넌트 불필요한 재렌더링 방지, useTransition으로 검색 입력 반응성 확보
- **태스크 관리(CRUD)**: 추가(컬럼별 생성, 제목·우선순위 필수, 설명 선택), 수정(모달), 삭제(확인 모달)
- **핵심 로직 유닛 테스트**: filterTasks, groupByStatus, restoreAtIndex, buildOptimisticTask, isLatestMutation 등 순수 함수 분리 후 테스트 (Vitest)

### Priority 2 (미구현)

## 미구현 사유

Priority 1을 견고하게 완성하는 데 집중하기 위해 Priority 2는 시간 관계상 구현하지 않았습니다.
